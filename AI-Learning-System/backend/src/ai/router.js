...
router.post('/chat', requireLogin, async (req, res, next) => {
  ...
  try {
    ...
    if (needsWebSearch(lastUser.content) && websearch.isEnabled()) {
      try {
        ...
        if (r.ok && r.results.length) {
          ...
          searchMeta = {
            sources: r.results.slice(0, 5).map(x => ({
              title: x.title,
              url:   x.url,
          ...
          logger.warn(`[ai/chat] web search failed: ${r.code}`);
        }
      } catch (err) {
    ...
    res.json({
      ...
      webSearch: searchMeta,   // { sources: [...], keyIndex, keyCount } or null
    });
