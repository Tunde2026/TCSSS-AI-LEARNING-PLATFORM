// backend/src/library/storage.js
...
function validateDocFile(file) {
  ...
  if (file.size > MAX_SIZE_BYTES) return { ok: false, reason: 'File is too large. Maximum is 100 MB.' };
  const ext = path.extname(file.originalname || '').toLowerCase();
  if (ext && !allowed.includes(ext)) return { ok: false, reason: `File extension "${ext}" does not match the file type.` };
  return { ok: true };
}
...
function libraryPathFor(filename) { return path.join(LIBRARY_DIR, filename); }

function removeFile(storagePath) {
  try {
    if (storagePath && fs.existsSync(storagePath)) {
      fs.unlinkSync(storagePath); return true;
    }
  } catch (_) {}
  return false;
}
...
function saveLogo({ file }) {
  ...
  try {
    ...
    for (const f of existing) { if (f.startsWith('logo.')) try { fs.unlinkSync(path.join(BRANDING_DIR, f)); } catch (_) {} }
  } catch (_) {}
...
module.exports = {
  ...
  MAX_SIZE_BYTES, MAX_LOGO_BYTES, ALLOWED_DOC_TYPES, ALLOWED_IMAGE_TYPES,
  ensureUploadDirs, validateDocFile, validateLogo, generateFilename,
  libraryPathFor, removeFile, saveManualEntry, saveLogo, slugify,
};